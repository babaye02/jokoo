// Formulaire d'avis client — accessible depuis /booking/detail/{id} quand
// la mission est marquée "completed" et qu'aucun avis n'a été laissé.
...
export default function LeaveReview() {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      // Le backend récupère provider_id via le booking.
      await api.post("/reviews", { booking_id: id, rating, comment: comment.trim() });
      router.replace({ pathname: `/booking/detail/${id}` as any, params: { just_reviewed: "1" } });
    } catch (e: any) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.surface2 }}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.xl, paddingBottom: 140 + insets.bottom }} keyboardShouldPersistTaps="handled">
        <Card>
          <Txt weight="700">Comment s&apos;est passée la mission ?</Txt>
        ...
        <Card style={{ marginTop: spacing.md }}>
          ...
          <TextInput
            ...
            multiline
            placeholder="Ponctualité, qualité du service, communication…"
